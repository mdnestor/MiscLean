...
-- A simplified Eckmann-Hilton argument (exercise 1.1.14 in "Introduction to Categorical Logic" by Awodey & Bauer, https://awodey.github.io/catlog/notes/catlogdraft.pdf)
...
example (h1: IsUnit m1 e) (h2: IsUnit m2 e) (h3: SwapCommute m1 m2): m1 = m2 := by
  ext x y
...
example (h1: IsUnit m e) (h2: SwapCommute m m): Assoc m := by
  intro x y z
  rw [←(h1 z).left]
  rw [h2]
  rw [(h1 x).right]
...
example (h1: IsUnit m e) (h2: SwapCommute m m): Comm m := by
  intro x y
